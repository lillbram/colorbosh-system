...
>
> ⚠️ **Singleton client (`global.__postgresClient`) WAJIB di-cache di semua environment, termasuk production** — jangan dibatasi `if (process.env.NODE_ENV !== "production")`. Ditemukan lewat debugging langsung setelah deploy pertama ke Vercel: guard itu awalnya ditulis dengan asumsi cache cuma perlu untuk bertahan dari HMR di dev, tapi di serverless (Vercel) modul ini bisa di-evaluasi ulang di instance yang sama antar request. Tanpa cache di production, setiap re-evaluasi bikin client `postgres()` BARU dengan pool `max: 8`-nya sendiri — beberapa request paralel saja sudah cukup untuk menembus batas 15 koneksi project, menyebabkan error acak "Server Components render" di halaman yang banyak query paralel (dashboard, `/production`, `/disbursement`, `/reports`, `/cash-flow`), sementara halaman dengan 1 query (`/procurement`, `/sales`) jarang kena. Fix: hapus guard `NODE_ENV`, selalu set `global.__postgresClient = client` di semua environment.
