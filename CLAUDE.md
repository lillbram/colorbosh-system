...
>
> ⚠️ **Serverless (Vercel) butuh DUA fix terpisah untuk pooling, bukan cuma satu** — ditemukan lewat debugging langsung setelah deploy pertama ke Vercel (halaman dengan banyak query paralel seperti dashboard, `/production`, `/disbursement`, `/reports`, `/cash-flow` gagal acak dengan "Server Components render" error):
...
> 2. **`max` pada client Postgres harus kecil (2), bukan besar (dulu 8)** — dan ini bukan sekadar redundan dengan fix #1. Di Vercel, request yang datang bersamaan bisa ditangani oleh BEBERAPA instance serverless SEKALIGUS, dan **setiap instance dapat budget `max` koneksinya sendiri-sendiri** — jadi `max` membatasi budget per-instance, bukan total aplikasi. Cuma beberapa instance aktif bersamaan di `max: 8` saja sudah cukup menembus batas 15 koneksi project. Diturunkan ke `max: 2` supaya beberapa instance bisa hidup bersamaan tanpa menembus batas, dengan headroom untuk Supabase Studio/script manual. Efek sampingnya: query yang dijalankan paralel lewat `Promise.all()` dalam SATU request jadi sedikit lebih sering antre (maks 2 jalan bersamaan, bukan 8) — trade-off yang wajar untuk skala 3 user, throughput dev lokal (1 instance) tidak terpengaruh.
